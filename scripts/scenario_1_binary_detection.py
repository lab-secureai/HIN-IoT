# -*- coding: utf-8 -*-
"""Primary-corpus binary malware detection experiments."""
...
DEFAULT_BASE_SAVE_DIR = '/content/drive/MyDrive/dataset/HIN/results_scenario_1_binary'
...
try:
    ...
    os.system("pip install -q torch_geometric lightgbm scikit-learn pandas openpyxl imbalanced-learn matplotlib seaborn")
    from torch_geometric.data import HeteroData
    from torch_geometric.loader import DataLoader as PyGDataLoader
    import torch_geometric.transforms as T
    from torch_geometric.nn import HeteroConv, SAGEConv, Linear, HGTConv, GATConv, HANConv, GCNConv
...
def load_and_split_dataset(file_path=DEFAULT_DATASET_PATH, seed=42, test_size=0.2):
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"Dataset file not found at: {file_path}")

    df = pd.read_csv(file_path)
    df['arch'] = df['arch'].apply(standardize_elf_architecture)

    if 'malware_family' not in df.columns:
        df['malware_family'] = 'Benign'
    df['malware_family'] = df['malware_family'].fillna('Benign')

    df['stratify_key'] = df['label'].astype(str) + "_" + df['arch'].astype(str)
    valid_counts = df['stratify_key'].value_counts()
    valid_keys = valid_counts[valid_counts > 1].index
    df_valid = df[df['stratify_key'].isin(valid_keys)].copy()

    df_train, df_test = train_test_split(
        df_valid, test_size=test_size, stratify=df_valid['stratify_key'], random_state=seed
    )
    return (
        df_train.drop(columns=['stratify_key']).reset_index(drop=True),
        df_test.drop(columns=['stratify_key']).reset_index(drop=True),
    )
...
def extract_streamlined_features(df_train, df_test):
    ...
    scaler_core_static = StandardScaler()
    x_tr_core_static = scaler_core_static.fit_transform(df_train[core_static_cols].apply(pd.to_numeric, errors='coerce').fillna(0))
    x_te_core_static = scaler_core_static.transform(df_test[core_static_cols].apply(pd.to_numeric, errors='coerce').fillna(0))
    ...
    X_tr_static_full = np.hstack([x_tr_core_static, x_tr_op])
    X_te_static_full = np.hstack([x_te_core_static, x_te_op])
    ...
    X_tr_dynamic_streamlined = x_tr_top_dyn
    X_te_dynamic_streamlined = x_te_top_dyn

    X_tr_hin_file = np.hstack([X_tr_static_full, X_tr_dynamic_streamlined])
    X_te_hin_file = np.hstack([X_te_static_full, X_te_dynamic_streamlined])
    ...
    preprocessors = {
        'scaler_core_static': scaler_core_static,
        'tfidf_op': tfidf_op,
        ...
        'baseline1_dim': x_tr_core_static.shape[1],
        'static_full_dim': X_tr_static_full.shape[1],
        'dynamic_streamlined_dim': X_tr_dynamic_streamlined.shape[1],
        'hin_file_dim': X_tr_hin_file.shape[1]
    }
    return (
        (x_tr_core_static, x_te_core_static),
        (X_tr_static_full, X_te_static_full),
        (X_tr_dynamic_streamlined, X_te_dynamic_streamlined),
        (X_tr_hin_file, X_te_hin_file),
        (X_tr_seq, X_te_seq),
        preprocessors,
    )


def build_hetero_graph_hin(df, X_hin_file, api_to_id=None, le_arch=None, mode='train'):
    def parse_api(s):
        try:
    ...
    api_col = 'api_sequence' if 'api_sequence' in df.columns else 'api_list'
    api_seqs = df.get(api_col, pd.Series([[]] * len(df))).apply(parse_api)
    ...
    if mode == 'train':
        ...
        arch_ids = le_arch.fit_transform(df['arch'].astype(str))
    else:
        known_archs = set(le_arch.classes_)
        arch_ids = df['arch'].apply(lambda x: le_arch.transform([str(x)])[0] if str(x) in known_archs else -1).values
    ...
    data = HeteroData()
    data['file'].x = torch.tensor(X_hin_file, dtype=torch.float)
    data['file'].y = torch.tensor(df['label'].values, dtype=torch.long)
    ...
    mask = arch_ids != -1
    edge_runs = np.array([np.arange(len(df))[mask], arch_ids[mask]])
...
def build_independent_ego_graphs(df, X_hin_file, api_to_id, le_arch):
    def parse_api(s):
        try:
            return ast.literal_eval(s) if isinstance(s, str) and s.startswith('[') else ([] if pd.isna(s) else str(s).split())
        except Exception:
    ...
    for i in range(len(df)):
        ...
        data['file'].y = torch.tensor([df['label'].iloc[i]], dtype=torch.long)
...
class MetaPathSAGE(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        h_file_final = (stacked_h * beta).sum(dim=1)
        return self.lin(h_file_final)
...
class DRHGT(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        z_disentangled = torch.cat([z_mal, z_ar], dim=-1)
        return self.classifier(z_disentangled)
...
def calculate_metrics(y_true, preds, probs, name, seed, train_ms=0.0, infer_ms=0.0, mem_mb=0.0):
    ...
    return {
        ...
        'PR-AUC': pr_auc,
        'ROC-AUC': roc_auc,
        'Train Time (ms/epoch)': train_ms,
        'Infer Time (ms/sample)': infer_ms,
        'Peak GPU VRAM (MB)': mem_mb
    }


def predict_hin_model(model, df_test, X_te_hin_file, apis, archs, device, eval_mode='mini_batch', batch_size=128):
    model.eval()
    if eval_mode == 'full_batch':
        test_graph, _, _ = build_hetero_graph_hin(df_test, X_te_hin_file, apis, archs, mode='test')
        ...
        bs = 1 if eval_mode == 'single_file' else batch_size
        graphs = build_independent_ego_graphs(df_test, X_te_hin_file, apis, archs)
        loader = PyGDataLoader(graphs, batch_size=bs, shuffle=False)
        all_probs, all_preds = [], []
        ...
        if device.type == 'cuda':
            torch.cuda.synchronize()
        infer_ms = (time.perf_counter() - t_start) * 1000 / len(df_test)
        probs, preds = np.array(all_probs), np.array(all_preds)
    return preds, probs, infer_ms
...
def run_scenario_1(
    ...
    seeds=DEFAULT_SEEDS,
    eval_mode='mini_batch'
):
    os.makedirs(base_save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Starting Scenario 1: Binary Malware Detection | Device: {device} | Eval Mode: {eval_mode}")
    ...
    for seed in seeds:
        ...
        df_train, df_test = load_and_split_dataset(dataset_path, seed=seed, test_size=0.2)
        y_tr, y_te = df_train['label'].values, df_test['label'].values
        print(f"Train samples: {len(df_train)} | Test samples: {len(df_test)}")

        (x_tr_core_stat, x_te_core_stat), (X_tr_stat_full, X_te_stat_full), (X_tr_dyn_stream, X_te_dyn_stream), (X_tr_hin_file, X_te_hin_file), (X_tr_seq, X_te_seq), prep = extract_streamlined_features(df_train, df_test)

        train_g, apis, archs = build_hetero_graph_hin(df_train, X_tr_hin_file, mode='train')
        ...
        joblib.dump(all_prep, os.path.join(seed_dir, 'preprocessors.pkl'))
        np.save(os.path.join(seed_dir, 'X_tr_hin_file.npy'), X_tr_hin_file)
        print("Training Baseline 1: Static LightGBM...")
        x_res, y_res = RandomUnderSampler(random_state=seed).fit_resample(x_tr_core_stat, y_tr)
        t0 = time.perf_counter()
        ...
        joblib.dump(lgbm, os.path.join(seed_dir, 'baseline_1_lgbm.joblib'))

        t_inf = time.perf_counter()
        p1, pr1 = lgbm.predict(x_te_core_stat), lgbm.predict_proba(x_te_core_stat)[:, 1]
        infer_ms = (time.perf_counter() - t_inf) * 1000 / len(y_te)
        all_raw_results.append(calculate_metrics(y_te, p1, pr1, "Baseline 1: Static (LightGBM)", seed, train_ms, infer_ms))
        print("Training Baseline 2: Dynamic Bi-GRU...")
        ...
        torch.save(bigru.state_dict(), os.path.join(seed_dir, 'baseline_2_bigru.pt'))

        p2, pr2, infer_ms_gru = predict_bigru_batched(bigru, X_te_seq, device, batch_size=256)
        all_raw_results.append(calculate_metrics(y_te, p2, pr2, "Baseline 2: Dynamic (Bi-GRU)", seed, train_ms, infer_ms_gru, peak_mem))
        clear_gpu_memory()
        ...
        torch.save(fusion_net.state_dict(), os.path.join(seed_dir, 'baseline_3_fusion.pt'))

        p3, pr3, infer_ms_fusion = predict_fusion_batched(fusion_net, X_te_seq, X_te_stat_full, device, batch_size=256)
        all_raw_results.append(calculate_metrics(y_te, p3, pr3, "Baseline 3: End-to-End Fusion", seed, train_ms, infer_ms_fusion, peak_mem))
        clear_gpu_memory()
        print("Training Baseline 4: Homogeneous Graph GCN...")
        ...
        torch.save(b4_model.state_dict(), os.path.join(seed_dir, 'baseline_4_gcn.pt'))

        p4, pr4, infer_ms_gcn = evaluate_inductive_homogeneous_gcn(b4_model, X_tr_hin_file, X_te_hin_file, device, k=5)
        all_raw_results.append(calculate_metrics(y_te, p4, pr4, "Baseline 4: Homogeneous GCN", seed, train_ms, infer_ms_gcn, peak_mem))
        clear_gpu_memory()
        def train_and_evaluate_hin_model(model_fn, model_filename, display_name, lr=0.01, epochs=100):
            ...
            if device.type == 'cuda':
                torch.cuda.synchronize()
            train_ms = (time.perf_counter() - t0) * 1000 / epochs
            peak_mem = torch.cuda.max_memory_allocated(device) / (1024 ** 2) if device.type == 'cuda' else 0.0

            torch.save(model.state_dict(), os.path.join(seed_dir, f"{model_filename}.pt"))

            preds, probs, infer_ms = predict_hin_model(model, df_test, X_te_hin_file, apis, archs, device, eval_mode=eval_mode)
            res = calculate_metrics(y_te, preds, probs, display_name, seed, train_ms, infer_ms, peak_mem)
            all_raw_results.append(res)
            print(f"Result -> {display_name} | Acc: {res['Accuracy']:.4f} | F1: {res['Binary F1']:.4f} | MCC: {res['MCC']:.4f}")
            clear_gpu_memory()

        train_and_evaluate_hin_model(lambda meta: BaseHeteroSAGE(hidden=64, out=2, metadata=meta), "HIN_RGCN", "Base HeteroSAGE")
        train_and_evaluate_hin_model(lambda meta: HGT(hidden=64, out=2, heads=4, layers=2, metadata=meta), "HIN_HGT", "HIN: HGT")
        train_and_evaluate_hin_model(lambda meta: HANModel(hidden=64, out=2, heads=4, metadata=meta), "HIN_HAN", "HIN: HAN")
        train_and_evaluate_hin_model(lambda meta: MetaPathSAGE(hidden=64, out=2, metadata=meta), "HIN_MAGNN", "Meta-path SAGE")
        train_and_evaluate_hin_model(lambda meta: GATModel(hidden=64, out=2, heads=4, metadata=meta), "HIN_GAT", "HIN: GAT")
        train_and_evaluate_hin_model(lambda meta: RHSAGE(hidden=64, out=2, metadata=meta), "IMPROVED_RGCN", "RH-SAGE")
        train_and_evaluate_hin_model(lambda meta: DRHGT(hidden=64, out=2, heads=4, layers=2, metadata=meta), "IMPROVED_HGT", "DR-HGT")

    df_raw = pd.DataFrame(all_raw_results)
    raw_csv_path = os.path.join(base_save_dir, "scenario_1_raw_results_10seeds.csv")
    ...
    summary_csv_path = os.path.join(base_save_dir, "scenario_1_summary_results_10seeds.csv")
