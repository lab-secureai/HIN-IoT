# -*- coding: utf-8 -*-
"""Evaluation of saved leave-one-architecture-out models on an external corpus."""
...
SAVED_WEIGHTS_DIR = '/content/drive/MyDrive/dataset/HIN/results_scenario_2_cross_isa'
EVAL_RESULTS_DIR = '/content/drive/MyDrive/dataset/HIN/results_f1_scenario_2_cross_isa'
...
def transform_new_dataset_features(df_new, prep_dict):
    core_cols = prep_dict['core_static_cols']
    top_dyn_cols = prep_dict['top_dyn_cols']

    for c in core_cols + top_dyn_cols:
        if c not in df_new.columns:
            df_new[c] = 0.0

    x_core = prep_dict['scaler_core_stat'].transform(df_new[core_cols].apply(pd.to_numeric, errors='coerce').fillna(0))
    op_col = 'opcode_counts' if 'opcode_counts' in df_new.columns else 'opcode_sequence'
    x_op = prep_dict['tfidf_op'].transform(df_new.get(op_col, pd.Series([""]*len(df_new))).fillna("").apply(parse_text_sequence)).toarray()
    X_stat_full = np.hstack([x_core, x_op])

    x_top_dyn = prep_dict['scaler_top_dyn'].transform(df_new[top_dyn_cols].apply(pd.to_numeric, errors='coerce').fillna(0))
    X_dyn_stream = x_top_dyn

    X_hin_file = np.hstack([X_stat_full, X_dyn_stream])

    api_col = 'api_sequence' if 'api_sequence' in df_new.columns else 'api_list'
    seq_texts = df_new.get(api_col, pd.Series([""]*len(df_new))).fillna("").apply(parse_text_sequence).tolist()
    X_seq = prep_dict['tokenizer'].texts_to_sequences(seq_texts, max_len=200)

    return x_core, X_stat_full, X_dyn_stream, X_hin_file, X_seq
...
def evaluate_on_new_dataset(
    ...
    print(f"Evaluating model benchmarks on dataset: {new_dataset_path}")
    ...
    for arch in architectures:
        ...
        if len(df_arch) == 0:
            continue
        y_true = df_arch['label'].values

        for seed in seeds:
            seed_dir = os.path.join(saved_weights_dir, f"arch_{arch}", f"seed_{seed}")
            prep_path = os.path.join(seed_dir, "preprocessors.joblib")
            if not os.path.exists(prep_path):
                continue

            artifacts = joblib.load(prep_path)
            prep = artifacts['preprocessors']
            api_to_id = artifacts['api_to_id']
            le_arch = artifacts['le_arch']
            X_tr_hin_file = artifacts['X_tr_hin_file']

            x_core, X_stat_full, X_dyn_stream, X_hin_file, X_seq = transform_new_dataset_features(df_arch, prep)
            lgbm_path = os.path.join(seed_dir, "baseline1_lgbm.joblib")
            if os.path.exists(lgbm_path):
                lgbm = joblib.load(lgbm_path)
                p_lgbm = lgbm.predict(x_core)
                pb_lgbm = lgbm.predict_proba(x_core)[:, 1]
                m = calculate_metrics(y_true, p_lgbm, pb_lgbm)
                m.update({'Architecture': arch, 'Model': 'Baseline 1: LightGBM', 'Seed': seed})
                all_results.append(m)
            b2_path = os.path.join(seed_dir, "baseline2_bigru.pt")
            if os.path.exists(b2_path):
                b2_net = Baseline2_BiGRU(vocab_size=prep['vocab_size']).to(device)
                b2_net.load_state_dict(torch.load(b2_path, map_location=device))
                p_b2, pb_b2, _ = predict_bigru_batched(b2_net, X_seq, device)
                m = calculate_metrics(y_true, p_b2, pb_b2)
                m.update({'Architecture': arch, 'Model': 'Baseline 2: Dynamic Bi-GRU', 'Seed': seed})
                all_results.append(m)
                clear_gpu_memory()
            b3_path = os.path.join(seed_dir, "baseline3_fusion.pt")
            if os.path.exists(b3_path):
                b3_net = Baseline3_EndToEndHybrid(vocab_size=prep['vocab_size'], dense_dim=X_stat_full.shape[1]).to(device)
                b3_net.load_state_dict(torch.load(b3_path, map_location=device))
                p_b3, pb_b3, _ = predict_fusion_batched(b3_net, X_seq, X_stat_full, device)
                m = calculate_metrics(y_true, p_b3, pb_b3)
                m.update({'Architecture': arch, 'Model': 'Baseline 3: End-to-End Fusion', 'Seed': seed})
                all_results.append(m)
                clear_gpu_memory()
            b4_path = os.path.join(seed_dir, "baseline4_gcn.pt")
            if os.path.exists(b4_path):
                b4_net = Baseline4_HomogeneousGCN(in_channels=X_hin_file.shape[1]).to(device)
                b4_net.load_state_dict(torch.load(b4_path, map_location=device))
                p_b4, pb_b4, _ = evaluate_inductive_homogeneous_gcn(b4_net, X_tr_hin_file, X_hin_file, device)
                m = calculate_metrics(y_true, p_b4, pb_b4)
                m.update({'Architecture': arch, 'Model': 'Baseline 4: Homogeneous GCN', 'Seed': seed})
                all_results.append(m)
                clear_gpu_memory()
            hin_models = [
                ('hin_rgcn.pt', 'Base HeteroSAGE', lambda meta: BaseHeteroSAGE(64, 2, meta)),
                ('hin_hgt.pt', 'HIN: HGT', lambda meta: HGT(64, 2, 4, 2, meta)),
                ('hin_gat.pt', 'HIN: GAT', lambda meta: GATModel(64, 2, 4, meta)),
                ('hin_han.pt', 'HIN: HAN', lambda meta: HANModel(64, 2, 4, meta)),
                ('hin_magnn.pt', 'Meta-path SAGE', lambda meta: MetaPathSAGE(64, 2, meta)),
                ('hin_improved_rgcn.pt', 'RH-SAGE', lambda meta: RHSAGE(64, 2, meta)),
                ('hin_improved_hgt.pt', 'DR-HGT', lambda meta: DRHGT(64, 2, 4, 2, meta))
            ]

            metadata = (['file', 'api', 'arch'], [
                ('file', 'calls', 'api'), ('api', 'rev_calls', 'file'),
                ('file', 'runs_on', 'arch'), ('arch', 'rev_runs_on', 'file')
            ])

            for weight_file, model_name, model_fn in hin_models:
                w_path = os.path.join(seed_dir, weight_file)
                if os.path.exists(w_path):
                    model = model_fn(metadata).to(device)
                    model.load_state_dict(torch.load(w_path, map_location=device))
                    p_hin, pb_hin, _ = predict_loao_hin(model, df_arch, X_hin_file, api_to_id, le_arch, device)
                    m = calculate_metrics(y_true, p_hin, pb_hin)
                    m.update({'Architecture': arch, 'Model': model_name, 'Seed': seed})
                    all_results.append(m)
                    clear_gpu_memory()

    df_res = pd.DataFrame(all_results)
    raw_path = os.path.join(output_dir, "evaluation_raw_results.csv")
    df_res.to_csv(raw_path, index=False)

    summary = []
    for (arch, model_name), group in df_res.groupby(['Architecture', 'Model'], sort=False):
        summary.append({
            'Architecture': arch,
            'Model': model_name,
            'Accuracy': f"{group['Accuracy'].mean():.4f} ± {group['Accuracy'].std():.4f}",
            'F1-Score': f"{group['F1-Score'].mean():.4f} ± {group['F1-Score'].std():.4f}",
            'PR-AUC': f"{group['PR-AUC'].mean():.4f} ± {group['PR-AUC'].std():.4f}",
            'ROC-AUC': f"{group['ROC-AUC'].mean():.4f} ± {group['ROC-AUC'].std():.4f}"
        })

    df_sum = pd.DataFrame(summary)
    sum_path = os.path.join(output_dir, "evaluation_summary_results.csv")
